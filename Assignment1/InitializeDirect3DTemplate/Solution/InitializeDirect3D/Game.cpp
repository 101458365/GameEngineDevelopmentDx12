...
void Game::BuildMaterials()
{
	...
	// ADD THESE TWO NEW MATERIALS:
	auto eagle = std::make_unique<Material>();
	eagle->Name = "Eagle";
	eagle->MatCBIndex = 5;
	eagle->DiffuseSrvHeapIndex = 0;
	eagle->NormalSrvHeapIndex = 1;
	eagle->DiffuseAlbedo = XMFLOAT4(0.3f, 0.5f, 0.9f, 1.0f);  // Blue
	eagle->FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
	eagle->Roughness = 0.3f;

	auto raptor = std::make_unique<Material>();
	raptor->Name = "Raptor";
	raptor->MatCBIndex = 6;
	raptor->DiffuseSrvHeapIndex = 0;
	raptor->NormalSrvHeapIndex = 1;
	raptor->DiffuseAlbedo = XMFLOAT4(0.9f, 0.2f, 0.2f, 1.0f);  // Red
	raptor->FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
	raptor->Roughness = 0.3f;

	mMaterials["bricks0"] = std::move(bricks0);
	mMaterials["tile0"] = std::move(tile0);
	mMaterials["mirror0"] = std::move(mirror0);
	mMaterials["sky"] = std::move(sky);
	mMaterials["Eagle"] = std::move(eagle);    // ← ADD
	mMaterials["Raptor"] = std::move(raptor);  // ← ADD
}
